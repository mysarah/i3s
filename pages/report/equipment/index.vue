<script setup>
definePageMeta({
  title: "Report equipment",
  
});

const form = ref({   
    School: "",
    Equipment: "",    
    Status:""    
})

const schoolDropdown = ref([  
  { label: "Select School", value: "" },
  { label: "SMK Seksyen 18 Shah Alam", value: "SMK Seksyen 18 Shah Alam" },
  { label: "MRSM Lenggong", value: "MRSM Lenggong" },
  { label: "Smk Taman Tun Dr Ismail", value: "Smk Taman Tun Dr Ismail" },
]);

const statusDropdown = ref([
  { label: "Choose status", value: "" },
  { label: "Active", value: "Active" },
  { label: "Faulty", value: "Faulty" },
  { label: "Repair", value: "Repair" },  
]);

const equipmentDropdown = ref([
  { label: "Choose equipment", value: "" },
  { label: "POS Terminal", value: "POS Terminal" },
  { label: "Reader", value: "Reader" },
  { label: "Server", value: "Server" }, 
  { label: "Switch", value: "Switch" },
])

const data = [
{   
    "#":"0001",     
    "Equipment Type": "RFID Reader",
    "Serial Number": "202306170100031",
    "Location":"SMK SEKSYEN 18",
    "Status":"Active",
    "Action":"",
  }, 
  {   
    "#":"0002",     
    "Equipment Type": "RFID Reader",
    "Serial Number": "202306170100033",
    "Location":"SMK SEKSYEN 18",
    "Status":"Active",
    "Action":"",
  },  
  {   
    "#":"0003",     
    "Equipment Type": "RFID Reader",
    "Serial Number": "202306170100036",
    "Location":"SK PUTRAJAYA PRESINT 18(2), PUTRAJAYA",
    "Status":"Active",
    "Action":"",
  }, 
  
]

</script>


<template>
<div>
    <div>
      <div class="flex justify-between">
        <h4 class="mb-4 uppercase">search equipment</h4>
      </div>
    </div>
<!-- SEACRH FUNCTION -->
   <rs-card class="p-5">
       <FormKit type="form" :actions="false" :incomplete-message="true" @submit="submitForm">
        <section>
          <!--<FormKit v-model="form.School" type="select"          
           :options="schoolDropdown" label="School Name" validation="required|length:0,50"  />-->                
                  
          <FormKit v-model="form.Equipment" type="select"          
            :options="equipmentDropdown"
            label="Equipment Type" 
            validation="required|length:0,50" />        
                  
          <FormKit v-model="form.Status" type="select"          
            :options="statusDropdown"
            label="Status" 
            validation="required|length:0,50" />
        </section>
       </FormKit>
     <div>
      
      <nuxt-link :to="`/myForm`">
        <rs-button variant="warning" class="w-64" style="margin-top: -0.5%;margin-left: 91%;width: 9%;">Search</rs-button>
      </nuxt-link>  
      <nuxt-link :to="`/myForm`">
        <rs-button variant="primary" class="w-64" style="margin-top: -2.8%;margin-left: 81%;width: 9%;">Clear</rs-button>
      </nuxt-link>
     </div>
    </rs-card>
    
    <rs-card class="p-5">
      <FormKit type="form" :actions="false" :incomplete-message="true" @submit="submitForm">
        <section>
            <rs-table
              :data="data"
              :options="{
               variant: 'default',
               striped: true,
               bordered: true,
               borderless: true,
               hover: true,
               fixed: false,
               }"
               advanced
               class="mt-4"
               >
               <template v-slot:Action="data">
                 <div class="flex flex-row gap-x-2">
                   <div class="flex items-center gap-x-2">
                      <nuxt-link :to="`/report/equipment/view-details/[id]`">
                        <rs-button
                        variant="warning"
                        >
                        <Icon name="ion:eye-outline" /> 
                        </rs-button>
                      </nuxt-link>
                    </div>            
                  </div>
                </template>
             </rs-table>
        </section>
      </FormKit>
    </rs-card>
  </div>        
</template>