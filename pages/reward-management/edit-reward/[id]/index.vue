...
const form = ref({
title : "SMK SEKSYEN 18 Cash Intensive April 2025",
amount : "10",
            ...
            <section>
                 <rs-card class="p-5">
                    <div class="flex justify-between items-center mb-5">
                        <div>
                            <h5 class="font-semibold">Edit Cash Reward</h5>
                            <span class="text-sm text-gray-500">                                
                            </span>
                        </div>
                    </div>
                    
                    <FormKit v-model="form.title" type="text" label="Title*" disabled = disabled />
                    <FormKit type="textarea" rows="8" label="Reward Description" disabled = disabled /> 
                    ...
                    <FormKit v-model="form.amount" type="text" label="Amount Reward" validation="required|length:0,50"/>
                  <div>
                     <fieldset>
                        <legend></legend>
                        ...
                        <FormKit v-model="form.title"              
                            type="select"                    
                            :options="programDetailDropdown"
                            label="Program details" help="Refer to Credit Setup Title"
                            validation="required|length:0,50" disabled = disabled />                        
                    </fieldset>
                 </div>
             

                    <FormKit v-model="form.minAmtReward" type="text" label="Minimum Amount Reward (RM)" validation="required|length:0,50"/>
                    <FormKit v-model="form.maxAmtReward" type="text" label="Maximum Amount Reward (RM)" validation="required|length:0,50"/>
                    <FormKit v-model="form.limit" type="text" label="Limit Amount per user (RM) *" validation="required|length:0,50"/>
                    <FormKit v-model="form.cycle" type="number" label="Cycle" help="Number of cycle to be applied for every user" validation="required|length:0,50"/>
                    ...
                    <FormKit v-model="form.Startdate" type="date" label="Start Date" validation="required|length:0,50" disabled = disabled />
                    <FormKit v-model="form.Enddate" type="date" label="End Date" validation="required|length:0,50" disabled = disabled />
                ...
                <div class="flex justify-between items-center mb-5">
                 ...
                 <fieldset>
                  ...
                  <!-- Date Time if choose cronjob type FREQUENCY -->
                    <FormKit v-model="form.frequency" type="text" label="Frequency value" validation="required|length:0,50"/>
            ...
            <section>
                <rs-card class="py-5">
                  <div
                    ...
                    <nuxt-link to="/campaign-management/reward-setup/add-school">
                ...
                <template v-slot:Action="dataSchool">
                    <div class="flex flex-row gap-x-2">
                        ...
                        <nuxt-link :to="`/campaign-management/reward-setup/edit-school/[id]`">
                            <rs-button
                            variant="warning"
                            >
                        ...
                        <div class="flex items-center gap-x-2">
                            <nuxt-link :to="`/campaign-management/reward-setup/delete-school/[id]`">
                                <rs-button
                                variant="danger">
