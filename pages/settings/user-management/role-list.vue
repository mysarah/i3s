...
<template>
  <div>
              ...
              <template v-slot:action="data">
                <div class="flex justify-center items-center" v-if="data.value.role?.value != '1'" >
                  ...
                  <Icon
                    ...
                    size="22" @click="openModalDelete(data.value)" ></Icon>
                </div>
    ...
    <rs-modal :title="modalType == 'edit' ? 'Edit Role' : 'Add Role'" ok-title="Save" :ok-callback="saveRole" v-model="showModal" >
      <FormKit type="text" v-model="showModalForm.name" label="Name" validation="required" validation-visibility="live"/>
      <FormKit type="textarea" v-model="showModalForm.description" label="Description" />
   
      <FormKit type="select" :options="statusDropdown" v-model="showModalForm.status" name="status" label="Status" />
    </rs-modal>

    <!-- Modal Role -->
    <rs-modal title="Add User" ok-title="Save" cancel-title="Back" :cancel-callback="closeModalUser" :ok-callback="saveUser" v-model="showModalUser" >
      <FormKit type="text" v-model="showModalUserForm.username" name="username" label="Username" />
      <FormKit type="text" v-model="showModalUserForm.fullname" name="fullname" label="Fullname"  />
      <FormKit type="text" v-model="showModalUserForm.email" name="email" label="Email" validation="email" validation-visibility="dirty"/>
      <FormKit type="mask" v-model="showModalUserForm.phone" name="phone" label="Phone" mask="###########" />

      <FormKit type="select" :options="statusDropdown" v-model="showModalUserForm.status" name="status" label="Status" />
    </rs-modal>

    <!-- Modal Delete Confirmation -->
    <rs-modal title="Delete Confirmation" ok-title="Yes" cancel-title="No" :ok-callback="deleteRole" v-model="showModalDelete" >
      <p>
